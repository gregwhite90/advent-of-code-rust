...
/// This solution works, but it is slow. It really should probably scan for a `*`
/// symbol and then only look at the part numbers adjacent to those symbols, adding up as it goes.
pub mod part_two {
    ...
    pub struct Soln {
        part_numbers: HashSet<PartNumber>,
        symbols: HashMap<char, HashSet<Point>>,
    }

    impl Solution for Soln {
        fn solve(&mut self, filename: &str) -> Answer {
            self.parse_input_file(filename);
            Answer::U32(self.sum_of_gear_ratios())
        }
    }
    ...
    impl Soln {
            ...
            io_utils::file_to_lines(filename).enumerate().for_each(|(row, line)| {
                ...
                for (col, ch) in line.chars().enumerate() {
                    if !ch.is_ascii_digit() { 
                        ...
                        cur_part_number = PartNumber::default();
                        if ch != '.' { 
                            self.symbols.entry(ch)
                                .and_modify(|set| {
                                    set.insert(Point { row, col });
                                })
                                .or_insert(HashSet::from([Point { row, col }]));
                        }
                    } else {
                        cur_part_number.val = cur_part_number.val * 10 + ch.to_digit(10).unwrap();
                        cur_part_number.points.insert(Point { row, col });
        ...
        fn sum_of_gear_ratios(&self) -> u32 {
            self.symbols.get(&'*').unwrap().iter().map(|pt| {
                let mut adj_p_ns = self.part_numbers
                    .iter()
                    .filter(|part_number| {
                        part_number.adjacent_points().contains(&pt)
                    });
                let mut gear_ratio = 0;
                if let Some(p_n) = adj_p_ns.next() {
                    gear_ratio += p_n.val;
                } else {
                    return 0;
                }
    ...
    mod tests {
        ...
        #[test_case(1, Answer::U32(467835); "example_1")]
        fn examples_are_correct(example_key: u8, answer: Answer) {
            test_utils::check_example_case(
                &mut Soln::default(),
                example_key,
