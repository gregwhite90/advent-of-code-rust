...
pub mod part_two {
    ...
    impl Soln {
        ...
        fn minimum_heat_loss(&self) -> u32 {
            ...
            let mut min_straight_steps_seen = HashMap::new();
            let path = Path {
                heat_loss: (1..=MIN_STEPS as usize).map(|step| self.heat_loss(&Point { row: 0, col: step })).sum(),
                position: Point { row: 0, col: MIN_STEPS as usize },
                direction: Direction::East,
                straight_steps: MIN_STEPS,
            };
            pq.push(path);
            let path = Path {
                heat_loss: (1..=MIN_STEPS as usize).map(|step| self.heat_loss(&Point { row: step, col: 0 })).sum(),
                position: Point { row: MIN_STEPS as usize, col: 0 },
                direction: Direction::South,
                straight_steps: MIN_STEPS,
            };
            pq.push(path);
        ...
        // TODO: figure out why the max_steps check is not triggering correctly and is allowing such a low
        // overall minimum heat loss
        fn next_position(&self, path: &Path, direction: Direction) -> Option<(Point, u32)> {
            if path.direction == direction.opposite() || path.direction == direction && path.straight_steps == MAX_STEPS {
                return None;
            } else {
                let steps = if path.direction == direction { 1 } else { MIN_STEPS as usize };                
                match direction {
                    Direction::North => {
