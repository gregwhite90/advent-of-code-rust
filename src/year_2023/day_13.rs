...
pub mod part_two {
    ...
    fn reflection_axis(
        ...
        let smudges_necessary = 1; // TODO: make more flexible
        for idxs in vec_to_vec.values() {
            if idxs.len() >= 2 {
    ...
    impl Pattern {
        ...
        fn reflection_axis(&self, axis: Axis) -> Option<usize> {
            ...
            if let Some(axis) = reflection_axis(vec_to_vec, total, idx_to_vec) {
                return Some(axis);
            }
            ...
            if check_axis(1, 0, total, idx_to_vec) {
                return Some(0);
            }
            if check_axis(1, total - 2, total, idx_to_vec) {
                return Some(total - 2);
            }
            None
        }
