...
mod utils {
    ...
    impl Factory {
        pub fn with_designated_values(designated_values: HashSet<u32>) -> Self {
            Self {
                designated_values,
                ...
                for (dest, val) in destinations.into_iter() {
                    match dest {
                        ...
                        Destination::OutputBin(id) => {
                            self.output_bins.entry(id).or_insert(OutputBin::default()).push_val(val);
                        }
                    }
                }
            }
        ...
        pub fn id_comparing_designated_values(&self) -> u32 {
            self.id_comparing_designated_values.unwrap()
        }
...
pub mod part_one {
    ...
    impl Solution for Soln {
        fn solve(&mut self, filename: &str) -> Answer {
            self.factory.parse_input_file(filename);
            self.factory.process();
            Answer::U32(self.factory.id_comparing_designated_values())
        }
    }
    ...
    impl Soln {
        fn with_designated_values(designated_values: HashSet<u32>) -> Self {
            Self {
                factory: Factory::with_designated_values(designated_values),
            }
        }
    }
...
pub mod part_two {
    ...
    impl Solution for Soln {
        fn solve(&mut self, filename: &str) -> Answer {
            self.factory.parse_input_file(filename);
            self.factory.process();
            Answer::U32(self.factory.output_value())
        }
    }
